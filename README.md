...
### 1. 用户抵押资产
`UserA` 将 **100 USDT** 抵押到 LendingPool。
```solidity
// 用户转账抵押资产
...
### 2. 计算抵押利率
接下来，池子计算存入的 **100 USDT** 的流动性回报率（即抵押利率）。假设当前池子的流动性率为 5%。
计算公式：
`liquidityRate = 5% * 100 USDT = 5 USDT;`
...
```
### 5. 更新借款和抵押的状态
池子中的抵押资产和借款总额将被更新，计算借款的利率、总的流动性等。池子内部将通过 `variableBorrowIndex` 和 `liquidityIndex` 更新相应的计算参数。
